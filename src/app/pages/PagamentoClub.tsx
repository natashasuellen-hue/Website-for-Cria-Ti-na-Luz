...
export function PagamentoClub() {
  const [searchParams] = useSearchParams();
  const planoId = searchParams.get("plano");
  const { siteData } = useData();
  const navigate = useNavigate();

  const plano = siteData.planosClub?.find((p) => p.id === planoId);

  if (!plano) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-primary mb-4">Plano não encontrado</h2>
          <Button onClick={() => navigate("/club")}>Voltar para o Club</Button>
        </div>
      </div>
    );
  }

  // Plano Free redireciona direto para WhatsApp
  if (plano.preco === "Grátis" && plano.linkWhatsApp) {
    window.location.href = plano.linkWhatsApp;
    return null;
  }

  const handleWhatsApp = () => {
    if (plano.linkWhatsApp) {
      window.open(plano.linkWhatsApp, "_blank");
    }
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white p-8 text-center">
            ...
            <h1 className="text-3xl mb-2">Assinar Plano {plano.nome}</h1>
            <p className="text-white/90">Complete sua assinatura do Cria Ti na Luz Club</p>
          </div>

          <div className="p-8">
            <div className="bg-muted rounded-xl p-6 mb-6">
              <h3 className="text-primary mb-4">Detalhes da Assinatura</h3>
              <div className="space-y-2">
                ...
                <div className="flex justify-between pt-4 border-t border-border">
                  ...
                  <span className="text-2xl text-primary">{plano.preco}/mês</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="bg-green-50 border-2 border-green-500 rounded-xl p-6 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Faça o Pix do valor mensal e envie o comprovante
                </p>
              </div>
              ...
              {plano.linkPayPal && (
                <Button
                  ...
                  onClick={() => window.open(plano.linkPayPal, "_blank")}
                >
              ...
              {plano.linkMercadoPago && (
                <Button
                  className="w-full bg-blue-400 hover:bg-blue-500 h-14"
                  onClick={() => window.open(plano.linkMercadoPago, "_blank")}
                >
